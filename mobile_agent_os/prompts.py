...
def app_user_prompt(
    ...
    *,
    ...
    return (
        ...
        "If a recent click already opened a detail screen and the requested information is visible, finish or respond instead of clicking the same topic again. "
        "If a visible record already matches part of the task, such as the right title, place, item, or time, edit or expand that existing record to fill missing details instead of creating a duplicate. "
        "When you need to edit missing details in an existing record, prefer the surrounding record/card/details row over nested value, toggle, or time controls. "
