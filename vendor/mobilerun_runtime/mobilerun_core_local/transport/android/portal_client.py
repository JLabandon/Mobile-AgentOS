...
from async_adbutils.errors import AdbTimeout

from mobilerun_core_local.driver.android.portal import PORTAL_PACKAGE_NAME, portal_content_uri
...
class PortalClient:
    ...
    async def _get_state_tcp(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Content-provider fallback already ran (or the TCP path itself
            # timed out) — don't retry, let the bound surface to the caller.
            raise
        except Exception as e:
    ...
    async def _get_state_content_provider(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Surface as TimeoutError so callers (framework / ui_with_recovery)
            # can treat a wedged on-device a11y serializer as a real timeout
            # instead of an opaque error dict.
            raise TimeoutError(
                f"portal content-provider state_full timed out after 15s: {e}"
            ) from e
        try:
