...
class StealthDriver:
    ...
    async def swipe(
        ...
        Requires an inner driver with raw ADB shell access; other backends
        (portal HTTP, iOS, visual-remote, cloud) fall back to their own swipe.
        """
        if getattr(self.inner, "device", None) is None:
    ...
    async def input_text(self, text: str, clear: bool = False, **kwargs: Any) -> bool:
        """Type text word-by-word with random delays between words.

        Extra keyword arguments (``stealth``, ``wpm``, …) are forwarded to the
        inner driver so the proxy stays signature-compatible with
        ``DeviceDriver.input_text``.
        """
        words = text.split(" ")

        for i, word in enumerate(words):
            ok = await self.inner.input_text(word, clear=(clear and i == 0), **kwargs)
            if not ok:
                return False

            if i < len(words) - 1:
                space_ok = await self.inner.input_text(" ", clear=False, **kwargs)
                if not space_ok:
                    return False
